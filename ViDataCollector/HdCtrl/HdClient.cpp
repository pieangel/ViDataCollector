...
void HdClient::OnDmSymbolCode(CString& sTrCode, LONG& nRqID)
{
	...
	if (nRepeatCnt > 0) {
		...
		for (int i = 0; i < nRepeatCnt; i++) {
			...
			SmProduct* product = SmMarketManager::GetInstance()->FindProduct(product_code);
			if (product) {
				product->AddSymbol(symCode);
			}
...
void HdClient::OnDmSymbolMaster(CString& sTrCode, LONG& nRqID)
{
	...
	SmSymbolManager* symMgr = SmSymbolManager::GetInstance();
	SmSymbol* sym = symMgr->FindSymbol((LPCTSTR)strData000.Trim());
	if (!sym) {
		OnTaskComplete(nRqID);
		return;
	}
...
void HdClient::OnAbroadSymbolMaster(CString& sTrCode, LONG& nRqID)
{
	...
	for (int i = 0; i < nRepeatCnt; i++)
	{
		...
		SmSymbolManager* symMgr = SmSymbolManager::GetInstance();
		SmSymbol* symbol = symMgr->FindSymbol((LPCTSTR)strSymbolCode.Trim());
...
int HdClient::GetAbDeposit(SmTaskArg& arg)
{
	...
	std::string temp;
	reqString.append("1");
	temp = VtStringUtil::PadRight(VtGlobal::LoginUserID, ' ', 8);
	// 아이디 
	reqString.append(temp);
...
void HdClient::OnRcvdAbroadChartData(CString& sTrCode, LONG& nRqID)
{
	...
	SmChartData* chart_data = chartDataMgr->CreateChartData(req);
...
void HdClient::RegisterAccount(CString accountNo)
{
	...
	CString strUserId = VtGlobal::LoginUserID.c_str();
