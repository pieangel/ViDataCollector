...
bool VtGlobal::SiseHogaEventEnable = true;

CString VtGlobal::GetMacAddress()
{
	CString strMACAddress = _T("");
// 	CxNetCardInfo NCI;
// 	strMACAddress = NCI.GetNetCardMACAddress();
...
SmSymbolSelector* VtGlobal::GetSymSelector(CWnd* wnd)
{
	return nullptr;
}
...
// 		{
// 			SmSymbol* symbol = section->SymbolList.front();
...
void VtGlobal::AddSymbol(SmSymbol* sym)
{
	if (!sym)
		return;
...
void VtGlobal::RemoveSymbol(SmSymbol* sym)
{
...
SmSymbol* VtGlobal::FindSymbol(SmSymbol* sym)
{
	for (auto it = FavoriteSymbolList.begin(); it != FavoriteSymbolList.end(); ++it)
	{
		SmSymbol* symbol = *it;
