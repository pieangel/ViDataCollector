...
public:
	...
	SmSymbol* AddSymbol(std::string symCode, std::string name);
	std::vector<SmSymbol*>& GetSymbolList() {
		return _SymbolList;
	}
	SmSymbol* GetRecentMonthSymbol();
	SmSymbol* GetNextMonthSymbol();
	...
	std::vector<SmSymbol*> GetRecentMonthSymbolOnOption();
	...
	void AddToYearMonth(std::string symbol_code, std::string name, SmSymbol* symbol);
private:
	// 소수점 자리수
	int _Decimal = 0;
	// 품목코드
